...
def process_video_job(job_id, video_path, job_role, username, email_address):
    ...
    try:
        ...
        if username:
            ...
            cur = conn.cursor()
            cur.execute(
                "INSERT INTO interview_results (username, job_role, score, feedback, transcript) VALUES (?, ?, ?, ?, ?)",
...
def db_update_job(job_id, status, result_dict=None):
    ...
    try:
        ...
        cur.execute(
            "UPDATE processing_jobs SET status=?, result_json=? WHERE job_id=?",
...
def upload():
    ...
    cur = conn.cursor()
    cur.execute(
        "INSERT INTO processing_jobs (job_id, status) VALUES (?, ?)",
        (job_id, 'processing')
    )
    conn.commit()
    conn.close()
...
def get_job_result(job_id):
    conn = connect_db()
    cur = conn.cursor()
    cur.execute("SELECT status, result_json FROM processing_jobs WHERE job_id=?", (job_id,))
    row = cur.fetchone()
    conn.close()
